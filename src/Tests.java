...
class ExamplesGame {
  ...
  void testSelect(Tester t) {
    ...
    test.selectPiece(new Posn(2, 3));
    t.checkExpect(test.selectedPiece.isPresent(), true);
    t.checkExpect(test.selectedPiece.map(p -> p.contains(new Posn(2, 3))).orElse(false), true);
    // make sure it's the same piece
    // (as this piece occupies both locations as specified by demoLevel).
    t.checkExpect(test.selectedPiece.map(p -> p.contains(new Posn(1, 3))).orElse(false), true);
  }
}
